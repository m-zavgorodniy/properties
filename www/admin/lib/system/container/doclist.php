...
function render() { 
	...
	<?	$this->extra_data_section_types = array();
		...
		$rs_section = db_mysql_query("SELECT id, title FROM section WHERE section_id IS NULL AND meta_site_id = '" . mysql_real_escape_string($this->site_id) . "'", $this->conn);
		while ($row = mysql_fetch_assoc($rs_section)) { //?>
	...
	function get_section($section_id, $level = 1, $branch_published = true, $branch_protected = false) {
		...
		$is_rows = mysql_num_rows($rs_section);
...
<?					}
		...
		<?					}
							...
							$rs_banner = db_mysql_query("SELECT f.id, f.name title
														 ...
														 ORDER BY f.id", $this->conn);
							while ($row_banner = mysql_fetch_assoc($rs_banner)) { ?>
								<div class="cm-section cm-section-folder">
