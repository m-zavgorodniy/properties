...
function text_date_str($date, $locale = 'ru_RU', $format = 'j M Y', $is_time = false) {
	if ($locale != 'ru_RU') {
		...
		return trim(date("j",$d).' '.$months[date("n",$d)-1].' '.("Y" == substr($format, -1)?date("Y",$d):'') . ($is_time?', '.date('G:i',$d):''));
	}
}

function text_date_of_week($date, $locale = 'ru_RU') {
	if ($locale != 'ru_RU') {
...
function text_left_cut($str, $len, $no_dots = false, $encoding = 'UTF-8') {
	if (mb_strlen($str, $encoding) > $len) { 
		...
		return rtrim(preg_replace("/[^\w]$/u", "", $res)) . (!$no_dots?'…':'');
	} else {
		return $str;
	}
}

function my_number_format($num, $locale = '') {
	if ('ru_RU' == $locale) {
...
function valid_date($str) {
	// !! TODO
	// !! this function can be used in back-office only, because of DATE_SEPARATOR and DATE_MONTH_FIRST defined in the back-office config
...
function redirect_301($redirect_url) {
	header("HTTP/1.1 301 Moved Permanently");
	// prevent browsers from caching this redirect (not the target page!)
...
function date_diff_days($date_from, $date_to, $is_nights = false) {
	if (version_compare(phpversion(), '5.3', '>=')) {
		$d_from = new DateTime($date_from);
		$d_to = new DateTime($date_to);
		return $d_to->diff($d_from)->format("%d") + ($is_nights?0:1);
	} else {
		return floor((strtotime($date_to) - strtotime($date_from))/86400) + ($is_nights?0:1);
	}
