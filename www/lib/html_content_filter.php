...
function typo_filter($str, $locale = '') {
	...
	$str = preg_replace("/<([^>]*)>/es", "'<'.str_replace('\"', '¬', \"\\1\").'>'", $str);
	$str = preg_replace( "/<script(.*?)<\/script>/ies", "'<script'.str_replace('\"', '¬', \"\\1\").'</script>'", $str); // escapes single qoutes inside the script (and everywhere). todo
