...
function create_thumbnail($source_filename, $output_filename, $w = 0, $h = 0, $crop = '', $no_overwrite = false) {
	global $config;
	if (!$crop) {
	...
	return $phpThumb->createThumbnailFile($output_filename, $no_overwrite);
}
...
function crop_wh($source_filename, $crop_w, $crop_h, $output_filename = '') {
	$phpThumb = new myPhpThumb();

	$phpThumb->setSourceFilename($source_filename);
	
	$phpThumb->setParameter('fltr', 'crop|' . $crop_w . '|' . $crop_w . '|' . $crop_h . '|' . $crop_h);
